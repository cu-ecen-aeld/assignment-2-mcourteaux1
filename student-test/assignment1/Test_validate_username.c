...
{
    // 1) Get the hardcoded username from my_username() function
    const char* hardcoded_username = my_username();

    // 2) Get the username from the configuration file
    char* config_username = malloc_username_from_conf_file();

    // 3) Use Unity assertion to verify both strings are equal
    TEST_ASSERT_EQUAL_STRING_MESSAGE(hardcoded_username, config_username, "Usernames do not match!");

    // Free dynamically allocated memory for username from config file
